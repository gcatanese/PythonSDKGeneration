...
class UpdateCompanyWebhookRequest(BaseModel):
    ...
    active: Optional[StrictBool] = Field(None, description="Indicates if the webhook configuration is active. The field must be **true** for us to send webhooks about events related an account.")
    additional_settings: Optional[AdditionalSettings] = Field(None, alias="additionalSettings")
    communication_format: Optional[StrictStr] = Field(None, alias="communicationFormat", description="Format or protocol for receiving webhooks. Possible values: * **soap** * **http** * **json** ")
    description: Optional[StrictStr] = Field(None, description="Your description for this webhook configuration.")
    filter_merchant_account_type: Optional[StrictStr] = Field(None, alias="filterMerchantAccountType", description="Shows how merchant accounts are filtered when configuring the webhook. Possible values: * **includeAccounts**: The webhook is configured for the merchant accounts listed in `filterMerchantAccounts`. * **excludeAccounts**: The webhook is not configured for the merchant accounts listed in `filterMerchantAccounts`. * **allAccounts**: Includes all merchant accounts, and does not require specifying `filterMerchantAccounts`.")
    ...
    __properties = ["acceptsExpiredCertificate", "acceptsSelfSignedCertificate", "acceptsUntrustedRootCertificate", "active", "additionalSettings", "communicationFormat", "description", "filterMerchantAccountType", "filterMerchantAccounts", "networkType", "password", "populateSoapActionHeader", "sslVersion", "url", "username"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpdateCompanyWebhookRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpdateCompanyWebhookRequest:
        """Create an instance of UpdateCompanyWebhookRequest from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return UpdateCompanyWebhookRequest.parse_obj(obj)

        _obj = UpdateCompanyWebhookRequest.parse_obj({
